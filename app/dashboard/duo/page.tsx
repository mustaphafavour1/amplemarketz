  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative">
        ...
        <motion.div
          animate={{ width: leftOpen ? 310 : 52 }}
          transition={{ duration: 0.3, ease: "easeInOut" }}
          className="shrink-0 flex flex-col border-r"
          style={{ borderColor: "#D4E4EE", background: "rgba(255,255,255,0.88)", backdropFilter: "blur(8px)", overflow: "hidden" }}
        >
          {leftOpen ? (
            /* Expanded left panel */
            <div style={{ width: 310, minWidth: 310 }}>
              {/* Tabs row + collapse button */}
              <div className="flex items-center border-b px-3 pt-3 justify-between" style={{ borderColor: "#D4E4EE" }}>
                <div className="flex">
                  <button
                    onClick={() => setActiveTab("pending")}
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium rounded-t cursor-pointer transition-colors ${activeTab === "pending" ? "text-[#374151] border-b-2 border-[#374151]" : "text-[#9CA3AF]"}`}
                  >
                    Pending <span className="bg-[#0F1923] text-white text-[10px] px-1.5 py-0.5 rounded-full">37</span>
                  </button>
                  <button
                    onClick={() => setActiveTab("autopilot")}
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-[12px] font-medium rounded-t cursor-pointer transition-colors ${activeTab === "autopilot" ? "text-[#374151] border-b-2 border-[#374151]" : "text-[#9CA3AF]"}`}
                  >
                    On autopilot <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background: "#D4E4EE", color: "#9CA3AF" }}>0</span>
                  </button>
                </div>
                <button
              ...
              <div className="flex items-center gap-2 px-3 py-2 border-b" style={{ borderColor: "#D4E4EE" }}>
                <button className="text-[10px] text-[#6B7280] border rounded px-2 py-1 hover:border-[#374151] transition-colors cursor-pointer" style={{ borderColor: "#D4E4EE" }}>Sort: Date ↕</button>
                <button className="text-[10px] text-[#6B7280] border rounded px-2 py-1 hover:border-[#374151] transition-colors cursor-pointer" style={{ borderColor: "#D4E4EE" }}>Filters</button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto">
                {leads.map((lead, i) => (
                  <motion.button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-3 border-b hover:bg-[#F0F6FF] transition-all cursor-pointer text-left group ${selectedId === lead.id ? "bg-blue-50 border-l-[3px] border-l-[#1E40AF]" : ""}`}
                    style={{
                      borderBottomColor: "#D4E4EE",
                      filter: selectedId !== lead.id ? "blur(0.8px) opacity(0.7)" : "none",
                      boxShadow: selectedId === lead.id ? "0 2px 8px rgba(30,64,175,0.1)" : "none",
                    }}
                  >
                    <Avatar initials={lead.avatar} size={30} />
                    <div className="flex-1 min-w-0">
                      <div className="text-[12px] font-semibold text-[#374151] truncate">{lead.name}</div>
                      <div className="text-[10.5px] text-[#9CA3AF] truncate">{lead.company} · {lead.title.split(" ").slice(0, 3).join(" ")}</div>
                    </div>
            ...
            /* Collapsed left panel — avatars only */
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      border: selectedId === lead.id ? "2px solid #1E40AF" : "2px solid transparent",
                    }}
                  >
                    {lead.avatar}
                  </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              style={{
                ...
                borderRadius: 16,
                border: "0.3px solid rgba(147,197,253,0.4)",
                padding: 28,
              }}
            >
              {/* Profile header */}
              ...
              <div className="grid grid-cols-3 gap-4 mb-5">
                {[
                  ...
                  <div key={ci} className="rounded-xl p-4" style={{ background: "rgba(248,250,255,0.8)" }}>
                    <div className="text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-wider mb-2.5">{col.title}</div>
                ...
                <div className="rounded-xl p-4" style={{ background: "rgba(248,250,255,0.8)" }}>
                  <div className="text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-wider mb-2.5">Signal</div>
                  <div className="mb-2">
              ...
              <div className="mb-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-[10.5px] text-[#9CA3AF]">4 steps · Mailbox rotation</span>
                </div>
                  ...
                  {STEP_CHANNELS.map((step, i) => (
                    <div key={i} className="flex items-center">
                      <motion.button
                        ...
                        whileHover={{ y: -1 }}
                        className="flex flex-col items-center px-4 py-3 rounded-xl min-w-[100px] cursor-pointer transition-all"
                        style={{
                          border: activeStep === i ? "1.5px solid #1E40AF" : "1px solid #D4E4EE",
                          background: activeStep === i ? "#EFF4FF" : "rgba(255,255,255,0.85)",
                          boxShadow: activeStep === i ? "0 4px 16px rgba(30,64,175,0.12)" : "none",
                        }}
                      >
                        <span className="text-[10px] text-[#9CA3AF] mb-1">{step.day}</span>
                        <step.icon size={16} className={activeStep === i ? "text-[#1E40AF]" : "text-[#6B7280]"} />
                        <span className={`text-[11px] font-medium mt-1 ${activeStep === i ? "text-[#1E40AF]" : "text-[#6B7280]"}`}>{step.channel}</span>
                      </motion.button>
                      {i < STEP_CHANNELS.length - 1 && (
                        <div className="flex flex-col items-center w-8">
                          <span className="text-[9px] text-[#9CA3AF] mb-0.5">+{[2, 3, 4][i]}d</span>
              ...
              <div className="flex items-center justify-between p-3 rounded-xl border"
                ...
                <span className="text-[10.5px] text-[#9CA3AF] flex items-center gap-1.5">
        ...
        <motion.div
          ...
          style={{
            borderColor: "rgba(172,200,215,0.4)",
        ...
        >
          <div style={{ width: 280, minWidth: 280 }}>
            <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: "#D4E4EE" }}>
              <h3 className="text-[12px] font-semibold text-[#374151] flex items-center gap-1.5">
            ...
            <div className="p-4 flex flex-col gap-3 border-b" style={{ borderColor: "#D4E4EE" }}>
              {DUO_THINKING.map((item, i) => (
                ...
                >
                  ...
                  <span className={`text-[11px] leading-snug ${item.done ? "text-[#6B7280]" : "text-[#374151]"}`}>{item.text}</span>
                </motion.div>
              ))}
            </div>

            <div className="p-4">
              <p className="text-[10px] font-semibold text-[#9CA3AF] uppercase tracking-wider mb-3">Similar leads to action next</p>
              {leads.slice(1, 3).map(lead => (
                <div key={lead.id} className="flex items-center gap-2.5 mb-3 p-2.5 rounded-lg border hover:bg-[#F8FAFF] transition-colors" style={{ borderColor: "#D4E4EE" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-[#9CA3AF] truncate">{lead.company}</div>
                  </div>
