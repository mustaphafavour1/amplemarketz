...
function StatCard({ value, label, delta, positive }: { value: number | string; label: string; delta: number; positive: boolean }) {
  return (
    <motion.div
      ...
      style={{ borderColor: "#D4E4EE", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}
    >
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5">
        ...
        <div className="grid grid-cols-[1fr_380px] gap-5">
          ...
          <div className="bg-white rounded-xl border p-5" style={{ borderColor: "#D4E4EE", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
            ...
            <div className="flex flex-col">
              {(stats.timeline as Array<{ type: string; text: string; time: string; action: string | null }>).map((item, i) => (
                ...
                >
                  <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${TYPE_COLOR[item.type] ?? "bg-[#9A9A9A]"}`} />
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    {item.action && (
                      <button
                        className="text-[12px] cursor-pointer whitespace-nowrap transition-all hover:bg-[#111827] hover:text-white"
            ...
            <button className="text-[12px] text-[#1E40AF] font-medium hover:underline mt-1 cursor-pointer">
              View all 47 signals →
            </button>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded-xl border p-4" style={{ borderColor: "#D4E4EE", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
              ...
              <div className="flex flex-col gap-3">
                {(stats.pendingApprovalCards as Array<{ name: string; company: string; channel: string; preview: string }>).map((card, i) => (
                  <div key={i} className="border rounded-lg p-3" style={{ borderColor: "#D4E4EE" }}>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ background: "#EDF4FB", color: "#5C5C5C" }}>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button className="h-6 px-2.5 rounded-md border text-[11px] text-[#5C5C5C] cursor-pointer" style={{ borderColor: "#D4E4EE" }}>Edit</button>
                    </div>
                  </div>
                ))}
              </div>
